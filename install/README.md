...
企业的 MySQL、PostgreSQL、Redis、Elasticsearch、Kafka、Prometheus、Grafana、HDFS、Harbor、DolphinScheduler、K8s 由 DBA/OPS 统一维护，Cube Studio 收敛为 **MLOps 控制面**，仓库内不再附带这些基础设施的部署清单。
...
启动前先用 `install/kubernetes/external-services.example.yaml` 准备 Secret/ConfigMap，把外部服务连接信息（DB / Redis / ES / Kafka / Prometheus / Grafana / HDFS / Harbor）注入到 `infra` 命名空间。
...
1. 准备外部 MySQL/PG、Redis、Harbor、Prometheus/Grafana、HDFS 连接
