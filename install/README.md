...
# Cube Studio：基础设施外部化（默认且唯一部署模式）

企业的 MySQL / PostgreSQL / Redis / Elasticsearch / Kafka / Prometheus / Grafana /
HDFS / Harbor / Rancher（K8s 控制面）/ DolphinScheduler / LiteLLM（LLM 网关）等基础设施
由 DBA / OPS 统一维护，Cube Studio 收敛为 **MLOps 控制面**，仓库内不再附带这些基础设施的部署清单。

## 定位

Cube Studio = Web/API + Notebook 管理 + 训练任务管理 + 模型注册 + 推理服务管理 + K8s 资源控制。

## 仓库已不再附带的部署目录

| 类别 | 已删除 | 由谁承担 |
|---|---|---|
| 元数据库 | `install/kubernetes/mysql/` | 企业 MySQL / PostgreSQL |
| 缓存 / 队列 | `install/kubernetes/redis/` | 企业 Redis |
| 监控 | `install/kubernetes/prometheus/`（含 grafana） | 企业 Prometheus + Grafana |
| 镜像仓库 | `install/kubernetes/harbor/` | 企业 Harbor |
| 日志栈 | `install/kubernetes/efk/` `ilogtail/` | 企业 ES / 日志平台 |
| 消息中间件 | `install/kubernetes/kafka/` | 企业 Kafka |
| K8s 集群部署 | `install/kubernetes/rancher/`（含 kubekey/rke2/批量ssh） | 企业 Rancher / 自有 K8s 控制面 |
| 数仓 / ETL | （历史已移除） | DolphinScheduler + Hadoop YARN + Spark |
| LLM 网关 | （未自带部署） | 企业 LiteLLM 网关服务 |
...
仅一个：`install/kubernetes/start.sh`（前身是 `start-lite.sh`，已统一）。

启动前先用 `install/kubernetes/external-services.example.yaml` 准备 Secret/ConfigMap，
把外部服务连接信息（DB / Redis / ES / Kafka / Prometheus / Grafana / HDFS / Harbor /
LiteLLM 等）注入到 `infra` 命名空间。
...
- **镜像仓库**：Harbor
- **K8s 集群**：复用企业 Rancher / 自建集群；本仓库不再提供集群引导工具
- **数据与模型产物**：HDFS（MinIO 仅保留为 argo artifact store 中转）
- **监控**：Prometheus + Grafana（复用现有监控体系）
- **ES / Kafka**：作为可选增强能力，按需启用
- **LLM 网关**：所有 OpenAI 兼容请求走企业 LiteLLM；推理服务镜像里把
  `OPENAI_API_BASE` 指向 LiteLLM 入口即可

## 实施顺序

1. 准备外部 MySQL/PG、Redis、Harbor、Prometheus/Grafana、HDFS、LiteLLM 等连接
2. 复制并修改 `external-services.example.yaml`，应用 Secret/ConfigMap
3. 执行 `bash install/kubernetes/start.sh <INGRESS_IP>`
4. 验证 Notebook / Pipeline / 推理服务可正常拉起
